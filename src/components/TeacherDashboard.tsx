                        ...
                        {getDayTimetable(selectedReportDay).map((entry) => {
                          ...
                          return (
                            <div 
                              ...
                              className="py-3 px-2.5 -mx-2.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors flex justify-between items-center text-xs cursor-pointer group"
                            >
                              <div className="flex items-center space-x-3">
                                <div className="w-8 h-8 rounded-lg bg-emerald-500/10 text-emerald-600 flex items-center justify-center font-black group-hover:scale-105 transition-transform">
                                  {subjectObj?.code || 'SUB'}
                                </div>
                                <div>
                                  <p className="font-extrabold text-slate-900 dark:text-white group-hover:text-emerald-600 transition-colors">{subjectObj?.name || 'Subject'}</p>
          ...
          onSendMessage={(teacher, subject) => {
            if (onSendEmail) {
              onSendEmail(
                teacher.email, 
                teacher.name, 
                `Inquiry about ${subject}`, 
                `Hello Teacher ${teacher.name},\n\nI have a question regarding the topic covered in the ${subject} lesson block...`, 
                'Announcement'
              );
              alert(`Simulated email sent successfully to ${teacher.name}!`);
            } else {
