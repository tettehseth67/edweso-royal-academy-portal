...
import { 
  ...
  Mail, MessageSquare, Upload, Plus, CheckCircle, TrendingUp, Eye, Receipt, Calendar
} from 'lucide-react';
import { 
    ...
    <div className="space-y-6 animate-fade-in text-slate-800 dark:text-slate-100" id="parent-portal-dashboard">
      ...
      <div className="flex flex-wrap border-b border-slate-200 dark:border-slate-800 gap-1 md:gap-0">
        {[
          ...
          { id: 'timetable', label: 'Class Timetable', icon: <Calendar size={13} /> },
      ...
      {activeParentTab === 'timetable' && (() => {
        const studentTimetable = timetable.filter(tt => tt.classId === activeStudent.classId);
        return (
            ...
            <div className={`p-4 rounded-xl border flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${
              ...
              <div>
                <h2 className="font-display font-extrabold text-lg tracking-tight text-slate-900 dark:text-white">
                  Lesson Calendar for {activeStudent.name}
                </h2>
                <p className="text-xs text-slate-400">
                  Select any course period below to audit lessons plans, study objectives, assigned homework, and covers.
                </p>
              </div>
              <div className="bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-lg text-emerald-600 text-xs font-bold">
                Class: {classes.find(c => c.id === activeStudent.classId)?.name || 'JHS 2'}
              </div>
            </div>
              ...
              {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].map((day) => {
                ...
                return (
                  <div key={day} className={`p-4 rounded-xl border flex flex-col justify-start min-h-[160px] ${
                    isDarkMode ? 'bg-slate-900 border-slate-850' : 'bg-white border-slate-200/80'
                  }`}>
                    <h4 className="font-bold text-xs uppercase tracking-wide border-b border-slate-100 pb-2 mb-3 text-slate-800 dark:text-slate-200">{day}</h4>
      ...
      {selectedTimetableEntry && (
        <TimetableDetailView
          ...
          onSendMessage={(teacher, subject) => {
            // Direct message link in parent dashboard
